...
images_ph = tf.placeholder(tf.float32, [None, 32, 32, 3])    #输入图像shape为[batch,32,32,3]   32*32像素 3个通道
...
for i in range(1000):   # 对模型进行训练
    image_train,labels_train = batch(images32,labels,128)   #从训练集中随机选取128张图片

    _,loss_value = session.run([train_step,loss], feed_dict={images_ph: image_train, labels_ph: labels_train})  # 每次运行train_step时，将之前所选择的数据，填充至所设置的占位符中，作为模型的输入
    #print("step: %d" %i)
    ...
    print("Step{0},   Accuracy:{1}    Loss:{2}".format(i,accuracy,loss_value))
for i in range(10):
    print("================================================================")
